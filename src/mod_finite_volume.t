         ...
         {do ix^DB=ixCmin^DB,ixCmax^DB\} 
           if(cminC(ix^D,ii) >= zero) then
             ...
             fC(ix^D,iw,idims)=(cmaxC(ix^D,ii)*fLC(ix^D, iw)-cminC(ix^D,ii)*fRC(ix^D,iw)&
                   +tvdlfeps*cminC(ix^D,ii)*cmaxC(ix^D,ii)*(wRC(ix^D,iw)-wLC(ix^D,iw)))&
      ...
      do iw=iwstart,nwflux
         if (flux_type(idims, iw) == flux_tvdlf) then
            fLC(ixC^S,iw) = 0.5d0 * (fLC(ixC^S,iw) + fRC(ixC^S,iw) - tvdlfeps * &
                 max(cmaxC(ixC^S,1), abs(cminC(ixC^S,1))) * &
                 (wRC(ixC^S,iw) - wLC(ixC^S,iw)))
         else
      ...
      do iw=1,nwflux
        if (flux_type(idims, iw) == flux_tvdlf) then
          ...
          f1L(ixC^S,iw)= half*(fLC(ixC^S, iw) + fRC(ixC^S, iw) &
                 -tvdlfeps*max(sR(ixC^S,index_v_mag), dabs(sL(ixC^S,index_v_mag))) * &
                 (wRC(ixC^S,iw)-wLC(ixC^S,iw)))
